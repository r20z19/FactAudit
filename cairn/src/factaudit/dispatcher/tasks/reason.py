...
from factaudit.dispatcher.contracts import parse_json_output, validate_reason_payload
from factaudit.dispatcher.prompting import (
    format_fact_ids,
    format_open_intents,
    load_prompt,
    render_prompt,
...
from factaudit.server.models import ProjectDetail
...
def run_reason_task(
    ...
    develop_running: bool = False,
    develop_max_running: int = 0,
) -> str:
    ...
    try:
        ...
        prompt = render_prompt(
            load_prompt(config.runtime.prompt_group, "reason.md"),
            {
                ...
                "develop_max_running": str(max(develop_max_running, 0)),
            },
        )
        ...
        if kind == "complete":
            if develop_running:
                pending = _json.dumps({"from": data["from"], "description": data["description"], "worker": worker.name})
                LOG.info(
                    "reason deferred complete because develop is running project=%s worker=%s execute_ms=%s total_ms=%s",
                    project.project.id,
                    worker.name,
                    execute_ms,
                    total_ms,
                )
                return f"pending_complete:{pending}"
