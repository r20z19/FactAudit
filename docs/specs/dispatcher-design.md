...
Dispatcher 是 FactAudit 的客户端执行器。它负责：

1. 协调项目容器生命周期
2. 给 Agent 下发明确任务
3. 代 Agent 调用 FactAudit API 写回图

Agent 不直接认领 Intent，不直接 heartbeat，不直接调用 FactAudit API。Agent 只接收 Dispatcher 下发的任务，返回结构化结果；Dispatcher 再决定如何请求 Server。
...
2. Dispatcher 是唯一的协议写入者和控制面；Agent 不 claim、不 heartbeat、不直接调用 FactAudit API。
...
codex exec --dangerously-bypass-approvals-and-sandbox --model "{env.CODEX_MODEL}" \
  -c 'model_provider="factaudit"' \
  -c 'model_providers.factaudit.name="factaudit"' \
  -c 'model_providers.factaudit.wire_api="responses"' \
  -c 'model_reasoning_effort="high"' \
  -c 'model_providers.factaudit.base_url="{env.CODEX_BASE_URL}"' \
  -c 'model_providers.factaudit.env_key="OPENAI_API_KEY"' \
  -- "{prompt}"
```
...
| 字段 | 必填 | 含义 |
| --- | --- | --- |
| `server` | 是 | FactAudit Server 的 base URL |
...
- 如果项目已从 Server 删除，Dispatcher 会把找不到对应项目的 `factaudit-dispatch-*` 容器视为 orphan，并执行 stop 清理
