...
TaskType = Literal["reason", "explore", "bootstrap", "develop"]
...
DEFAULT_PROMPT_REQUIRED_TOKENS: dict[str, tuple[str, ...]] = {
    ...
    "develop.md": ("{graph_yaml}", "{intent_id}", "{intent_description}"),
    "develop_conclude.md": ("{graph_yaml}", "{intent_id}", "{intent_description}"),
}
...
PROMPT_REQUIRED_TOKENS_BY_GROUP: dict[str, dict[str, tuple[str, ...]]] = {
    "mock": {
        ...
        "bootstrap.md": ("{origin}", "{goal}", "{hints}"),
        "bootstrap_conclude.md": ("{origin}", "{goal}", "{hints}"),
        "develop.md": ("{intent_id}",),
        "develop_conclude.md": ("{intent_id}",),
    }
}
...
class DevelopTaskConfig(BaseModel):
    timeout: int = Field(gt=0)
    conclude_timeout: int = Field(gt=0)
...
class ContainerVolume(BaseModel):
    ...
    @field_validator("host_source")
    @classmethod
    def validate_source_non_empty(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("host_source must not be empty")
        return value.strip()

    @field_validator("container_target")
    @classmethod
    def validate_target_absolute(cls, value: str) -> str:
        text = value.strip()
        if not text.startswith("/"):
            raise ValueError("container_target must be an absolute path")
        return text
...
def validate_prompt_resources(prompt_group: str) -> None:
    prompts_dir = resources.files("factaudit.dispatcher.prompts")
