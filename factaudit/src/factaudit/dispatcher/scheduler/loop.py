...
from factaudit.dispatcher.config import DispatchConfig, WorkerConfig
...
from factaudit.dispatcher.runtime.startup_healthcheck import format_failure_summary, run_startup_healthchecks
from factaudit.dispatcher.scheduler.worker_select import choose_worker
from factaudit.dispatcher.tasks.bootstrap import run_bootstrap_task
from factaudit.dispatcher.tasks.develop import run_develop_task
from factaudit.dispatcher.tasks.explore import run_explore_task
from factaudit.dispatcher.tasks.reason import run_reason_task
from factaudit.server.models import Intent, ProjectDetail, ProjectSummary
...
class DispatcherLoop:
    def __init__(self, config_path: Path):
        ...
        self.client = FactAuditClient(self.config.server)
        ...
        self._pending_completes: dict[str, dict[str, object]] = {}
        self._develop_idle_since: dict[str, float] = {}
    ...
    def _try_dispatch_project(self, summary: ProjectSummary) -> bool:
        ...
        if container_name in self._cleanup_pending:
            ...
            return False
        if summary.id in self._pending_completes:
            self._log_changed(
                f"{skip_scope}:pending_complete",
                logging.DEBUG,
                "skip project=%s because pending complete is waiting for develop",
                summary.id,
            )
            return False
        ...
        running_develop_ids = self._project_running_develop_intents(summary.id)
        all_running_intents = running_intent_ids | running_develop_ids
        unclaimed_intents = [
            intent
            for intent in project.intents
            if intent.to is None
            and intent.worker is None
            and intent.id not in all_running_intents
            and not self._is_bootstrap_intent(intent)
        ]
        develop_unclaimed = [
            intent for intent in unclaimed_intents if self._is_develop_intent(intent)
        ]
        if develop_unclaimed:
            newest = max(develop_unclaimed, key=lambda i: i.created_at)
            export_yaml = self.client.export_project(summary.id)
            return self._dispatch_develop(project, export_yaml, newest)
        explore_unclaimed = [
            intent for intent in unclaimed_intents if not self._is_develop_intent(intent)
        ]
        if all_running_intents and not explore_unclaimed:
            self._log_changed(
                f"{skip_scope}:explore_running",
                logging.DEBUG,
                "skip explore project=%s because all unclaimed intents are already running locally intents=%s",
                summary.id,
                sorted(all_running_intents),
            )
        if explore_unclaimed:
            newest = max(explore_unclaimed, key=lambda i: i.created_at)
            export_yaml = self.client.export_project(summary.id)
    ...
    def _dispatch_reason(self, project: ProjectDetail, export_yaml: str, trigger: str) -> bool:
        ...
        if not claim.ok:
            ...
            return False
        try:
            develop_running = bool(self._project_running_develop_intents(project.project.id))
            develop_max_running = sum(
                w.max_running for w in self.config.workers if "develop" in w.task_types
            )
            future = self.executor.submit(
                ...
                develop_running=develop_running,
                develop_max_running=develop_max_running,
            )
        except Exception:
        ...
        self.futures[future] = RunningTask(
            ...
            open_intent_count=self._project_non_develop_open_intent_count(project),
        )
    ...
    def _dispatch_develop(self, project: ProjectDetail, export_yaml: str, intent: Intent) -> bool:
        selection = self._select_worker(project.project.id, "develop")
        worker = selection.worker
        if worker is None:
            self._log_changed(
                f"project:{project.project.id}:worker:develop",
                logging.INFO,
                "no worker available for develop project=%s intent=%s blocked_busy=%s blocked_unhealthy=%s blocked_rejected=%s",
                project.project.id,
                intent.id,
                selection.blocked_busy,
                selection.blocked_unhealthy,
            ...
            return False
        self._clear_log_state(f"project:{project.project.id}:worker:develop")
        claim = self.client.heartbeat(project.project.id, intent.id, worker.name)
        if claim.status_code in (403, 409):
            level = logging.INFO if claim.status_code == 403 else logging.WARNING
            LOG.log(
                level,
                "develop claim failed project=%s intent=%s worker=%s status=%s",
                project.project.id,
                intent.id,
                worker.name,
                claim.status_code,
        ...
        try:
            ...
            LOG.exception("failed to submit develop task project=%s intent=%s worker=%s", project.project.id, intent.id, worker.name)
            self._best_effort_release(project.project.id, intent.id, worker.name)
            return False
        self.futures[future] = RunningTask(project.project.id, "develop", worker.name, cancellation, intent_id=intent.id)
        self.runtime_project_ids.add(project.project.id)
        self._clear_project_log_state(project.project.id)
        LOG.info("dispatched develop project=%s intent=%s worker=%s", project.project.id, intent.id, worker.name)
        return True
    ...
    def _project_non_develop_open_intent_count(self, project: ProjectDetail) -> int:
        return sum(
            1 for intent in project.intents
            if intent.to is None and not self._is_develop_intent(intent)
        )
    ...
    @staticmethod
    def _is_develop_intent(intent: Intent) -> bool:
        return intent.description.startswith(DEVELOP_INTENT_PREFIX)

    def _project_running_develop_intents(self, project_id: str) -> set[str]:
        return {
            task.intent_id
            for task in self.futures.values()
            if task.project_id == project_id and task.task_type == "develop" and task.intent_id is not None
        }
    ...
    def _reason_trigger(self, project: ProjectDetail) -> str | None:
        ...
        if changes:
            self._develop_idle_since.pop(f"develop_idle:{project.project.id}", None)
            return ",".join(changes)

        non_develop_open = self._project_non_develop_open_intent_count(project)
        running_develops = bool(self._project_running_develop_intents(project.project.id))
        key = f"develop_idle:{project.project.id}"
        if non_develop_open == 0 and running_develops:
            now = time.time()
            if key not in self._develop_idle_since:
                self._develop_idle_since[key] = now
                return None
            if now - self._develop_idle_since[key] > 30:
                self._develop_idle_since.pop(key, None)
                return "develop_idle"
            return None
        self._develop_idle_since.pop(key, None)
        return None
    ...
    def _reap_futures(self) -> None:
        ...
        for future in done:
            ...
            try:
                ...
                elif outcome is not None and outcome.startswith("pending_complete:"):
                    pending_json = outcome.split(":", 1)[1]
                    try:
                        pending_data = _json.loads(pending_json)
                    except Exception:
                        LOG.warning(
                            "failed to parse pending complete data project=%s task=%s worker=%s",
                            task.project_id,
                            task.task_type,
                            task.worker_name,
                        )
                        continue
                    self._pending_completes[task.project_id] = pending_data
                    LOG.info(
                        "reason deferred complete project=%s; cancelling develop tasks",
                        task.project_id,
                    )
                    for dev_future, dev_task in list(self.futures.items()):
                        if dev_task.project_id != task.project_id:
                            continue
                        if dev_task.task_type != "develop":
                            continue
                        if dev_task.cancellation.cancel("project_completed", force_conclude=True):
                            LOG.info(
                                "cancelling develop for deferred complete project=%s develop_intent=%s",
                                task.project_id,
                                dev_task.intent_id,
                            )
                    continue
        ...
        self._check_pending_completes()

    def _check_pending_completes(self) -> None:
        for project_id in list(self._pending_completes.keys()):
            dev_running = any(
                t.project_id == project_id and t.task_type == "develop"
                for t in self.futures.values()
            )
            if dev_running:
                continue
            pending = self._pending_completes.pop(project_id)
            LOG.info(
                "executing deferred complete project=%s",
                project_id,
            )
            try:
                ...
                if response.ok:
                    LOG.info(
                        "deferred complete succeeded project=%s worker=%s",
                        project_id,
                        pending.get("worker"),
                    )
                else:
                    LOG.warning(
                        "deferred complete failed project=%s status=%s body=%s",
                        project_id,
                        response.status_code,
                        response.text,
                    )
            except Exception:
                LOG.exception("deferred complete crashed project=%s", project_id)
