...
class ContainerManager:
    _PREFIX = "factaudit-dispatch-"
    _STARTUP_PREFIX = "factaudit-startup-healthcheck-"
    ...
    def _build_volumes(self) -> dict[str, dict[str, str]] | None:
        if not self._config.volumes:
            return None
        result: dict[str, dict[str, str]] = {}
        for vol in self._config.volumes:
            mode = "ro" if vol.read_only else "rw"
            result[vol.host_source] = {"bind": vol.container_target, "mode": mode}
        return result
